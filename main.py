...
# Function to process the data and count KGBVS attendance by state
def process_data(df, medium):
    state_order = ["Telangana", "Andhra Pradesh", "Manipur", "Mizoram", "Tripura", 
                   "UP", "Gujarat", "Jharkhand", "Rajasthan", "Madhya Pradesh", 
                   "Haryana", "Himachal Pradesh"]
    
    # Filtering data based on language/medium
    filtered_df = df[df['Language'] == medium]
    
    # Count the KGBVS attendees by state
    state_counts = filtered_df.groupby('State')['KGBVS who attended'].sum().reindex(state_order, fill_value=0)
    
    # Create the CSV file
    csv_file_path = 'state_attendance.csv'
    filtered_df.to_csv(csv_file_path, index=False)
    
    return state_counts, csv_file_path
...
# Main Streamlit app
st.title('KGBVS Attendance Data Analyzer')

# File uploader
uploaded_file = st.file_uploader("Upload your CSV or Excel file", type=['csv', 'xlsx'])

# Dropdown for selecting language/medium
medium = st.selectbox('Select Medium/Language', ['Telugu', 'English', 'Hindi', 'Bengali'])
...
if uploaded_file:
    ...
    additional_states = {state: data[data['State'] == state]['KGBVS who attended'].sum() 
                         for state in data['State'].unique() if state not in state_order}
    ...
    if state_counts is not None:
        ...
        with open(csv_file_path, "rb") as f:
            ...
            st.markdown(f'<a href="data:file/csv;base64,{csv_base64}" download="{csv_file_path}">Download CSV Report</a>', unsafe_allow_html=True)
